...
## 2026-09-26 — Rule suggestions from the B-107 review applied (issue #15)
- chose: the human answered issue #15 "A apply, B apply, C apply". Applied verbatim: (A) DESIGN.md "Type" now says Fredoka One is used for headings, tiles, buttons and the English word wherever it is shown (tray, boxes, round-end list, sticker book); (B) DESIGN.md "Mascot / character" now says unearned stars are drawn as accent-colour outlines (not muted), so the three-star target is always visible; (C) BACKLOG.md B-108 acceptance says round-end shown (tada) fires once per round-end render, not on every setState while the screen is showing. Issue #13 (B-106 review) was consumed by the previous run.
- because: question protocol; answers are consumed at the start of the next run and logged here. None of the answers blocks or unblocks a backlog item.
- reversible: yes (doc lines).

## 2026-09-26 — Wrong feedback built on main; B-106's heart and afterMove hooks re-added byte-identical (B-109)
- chose: B-106 (PR #12) is not merged, so B-109 branches from main and re-adds only the pieces it depends on, copied byte for byte from the B-106 branch and at the same places: `renderHearts(lives, status)` with the `losing` class in `card.js`, `afterMove(status)` and the two `status: afterMove(current.status)` setState lines in `drag.js`, and the `.hearts .lost` transition plus `.hearts .lost.losing` rules in `game.css`. Not copied: `check.js`, the `onCheck` rewrite, `renderBoxes(count, status)` and the `.letter-boxes.correct` rule (B-109 does not need them). New B-109 CSS sits after `.letter-tray`, away from B-106's insertion point. A scratch merge in both directions confirms `card.js`, `drag.js` and `game.css` merge without conflict; the files that do conflict (CLAUDE.md, DESIGN.md, DECISIONS.md, tests.py, main.js imports, the `game.css?v=` line) already conflict between B-106 and plain main, and B-109 adds no new conflict hunk to them.
- because: the acceptance ("the lost heart, already error-coloured from B-106, fades to muted when the kid next moves a tile") needs those hooks to exist on this branch; identical lines are the only way to avoid a conflict with an open PR.
- wrong tint: a box is `wrong` when `current.status === 'wrong'` and the placed tile's letter differs from `current.word[i]` (`isWrongBox(current, i)` in `card.js`, applied in the placed loop; no state on DOM elements). The tile inside the box takes the error colour too, as B-104 did for filled/primary, because the tile covers the box. It clears on the next tile move because `afterMove` turns 'wrong' back into 'playing' and the tint is a pure render of that status; the same setState drops `losing` from the heart, whose CSS transition fades it to muted over 200ms. Correctly placed letters in a wrong arrangement stay untinted so the kid sees what to keep.
- wiggle: a new `static/js/feedback.js` subscriber (`renderFeedback`, wired in `main.js` after `renderCard`) keeps a render cache `last = { status, lives }` and starts the wiggle only when status becomes 'wrong', or stays 'wrong' while `lives` drops (Check pressed again without moving a tile, which B-106's Check allows). It is not triggered from `onCheck` because B-106 owns that function and because effects keyed on state transitions stay correct no matter who calls setState (a future correct-pulse/confetti in B-110 can use the same module). The tween is a GSAP timeline 0 -> -6 -> +6 -> -6 -> 0 on `x`, 300ms total, `power1.inOut`, from `MOTION.wiggle` in `motion.js`; `motionFor('wiggle', true)` returns duration 0 and distance 0 (reduced motion: tint only). It kills only the tile's `x` tweens first so a still-landing snap keeps its scale; a drag started mid-wiggle kills the tween (drag.js already does `killTweensOf`). Input is never blocked.
- considered: triggering the wiggle from `onCheck` (conflicts with B-106 and ties an effect to one caller); keying the effect on the `wrong` DOM class instead of state (couples two renderers); a CSS keyframe wiggle on `.letter-box.wrong` (would replay on every render while wrong and fight Draggable's transform); returning `distance: 0` for every reduced spec (changes the B-105 test contract for pickup/snap/return for no gain).
- reversible: yes.
