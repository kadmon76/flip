...
## 2026-09-22 — Round-end layout, star and mark rendering, Play again via startRound (B-107)
- chose: `#screen-round-end` mirrors the play screen: top-aligned column, `padding-bottom: 96px` (`--mascot-corner`) and the action row pushed down with `margin-top: auto`, so Play again / Themes sit just above the free bottom-left strip. Stars are three 64px inline SVGs in the accent colour, outline only (stroke) until earned, then filled (class `earned`, toggled by `renderRoundEnd`). Each word is a 48px white strip with a 40px thumbnail, the word in Fredoka One and a tick (accent) or cross (error colour) drawn as inline SVG; thumbnails come from `state.words[i].image` since `round.results[i]` holds only `{ word, correct }`. "Play again" looks up the current theme entry in `state.themes` and calls the existing `startRound`, which re-fetches the data file (browser cache), picks new random words and resets hearts and results; a new `#themes-btn` "Themes" handler resets theme, words, round and current and shows the theme screen (hearts are reset by `startRound`, so the handler does not touch `lives`). `starsFor(correct, size)` is written against `size` (all right -> 3, one short -> 2, at least 40% -> 1, else 0; the 5-word bands from the backlog) and throws on a non-integer or out-of-range score.
- because: DESIGN reserves the bottom-left corner on the round-end screen and B-104 already established the strip approach for full-width buttons; the same shape keeps both screens consistent. Inline SVG (as with the hearts) is the only way the accent colour is guaranteed on Android, where glyphs fall back to the colour-emoji font. Reusing `startRound` keeps one code path for "new round in theme X" and honours the state-only architecture. Tick in accent and cross in error match the palette roles ("correct", "lost heart"). The B-106 branch replaces `LIVES` with `HEARTS` in this file, so the new handlers avoid that constant to keep the merge small.
- considered: caching the theme's data in state to avoid the re-fetch (more state for no visible gain); a star glyph (colour-emoji fallback problem); a muted outline for unearned stars (accent outline reads as "three to collect", which is the reward the kickoff asked for).
- reversible: yes.

## 2026-09-25 — Rule suggestions from the B-107 review applied (issue #15)
- chose: the human answered issue #15 "A apply, B apply, C apply". Applied verbatim: (A) DESIGN.md "Type" now says Fredoka One is used for headings, tiles, buttons and the English word wherever it is shown (tray, boxes, round-end list, sticker book); (B) DESIGN.md "Mascot / character" now says unearned stars are drawn as accent-colour outlines (not muted), so the three-star target is always visible; (C) BACKLOG.md B-108 acceptance says the round-end tada fires once per round-end render, not on every setState while the screen is showing.
- because: question protocol; answers are consumed at the start of the next run and logged here. No item was blocked on #15, so nothing to unblock.
- reversible: yes (doc lines).

## 2026-09-25 — Sounds are cued from state changes, not DOM events (B-108)
- chose: `audio.onState` is a state subscriber. It keeps a memo of the last slice it heard (`screen`, `lives`, `current`, `muted`; a render-style cache like `builtThemes` in main.js, not game state) and asks the pure `cuesFor(prev, next)` which clips to play: `tada` when `screen` becomes `round-end` (only on the transition, so later setStates on that screen are silent); `correct` when `current.status` becomes `correct`; `error` whenever `lives` drops (every wrong check, including the third that reveals); `swipe` when some `placed[i]` holds a new or different tile and the status is not `revealed` (the reveal fills every box and plays only `error`; a tile going back to the tray is silent). Two channels: voice (`correct`, `error`, `tada`), where a new clip stops the one playing, and fx (`swipe`), which may overlap the voice. `pickIndex` never returns the clip played last when the pool has more than one.
- because: the acceptance says tap-to-place and drag drop both cue the swipe via the state change, and CLAUDE.md says every screen is a render of state; one diff serves both input paths and `--eval`, and the "once per transition" rule for tada falls out of comparing the previous screen. `cuesFor`, `clipUrl`, `pickIndex`, `readMuted`/`writeMuted` are DOM-free and node-tested (`AudioTests`), including a check that `FILES` lists exactly the mp3s on disk so a renamed clip fails the suite. Tada is treated as a voice clip because it is the character celebrating; DESIGN says a new voice clip cuts the previous one, so it cuts a still-playing "correct" line when the kid taps Next.
- considered: calling `play()` from `onCheck`, `onNext` and drag.js (two code paths for placing a tile; the round-end cue would sit in the Next handler rather than follow the screen); keeping the previous state inside state.js (would put a copy of the state into the state).
- reversible: yes.

## 2026-09-25 — Pools are a spelled-out file list, preloaded; first gesture primes every clip muted (B-108)
- chose: `FILES` in audio.js names every clip under `correct/`, `error/`, `celebration/tada.mp3` and `swipe/`; `clipUrl` builds `/static/sounds/<dir>/<encodeURIComponent(name)>`. `init()` creates one `Audio` per clip with `preload="auto"` and never calls `play()`, so a load produces no autoplay-policy error. The first `pointerdown` or `keydown` (once, capture) runs `unlock()`: every element is set `muted`, `play()`ed and paused again at `currentTime = 0` when the promise resolves; an element that `play(kind)` claims in the meantime is dropped from the priming set so the real clip is not paused. Every `play()` promise has a `catch` so a browser that still refuses stays silent without a console error. `background/` and `themes/` are not pooled (no background music in M1; per-word audio is M2).
- because: the browser cannot list `static/sounds/`, and the content is read-only in M1, so a static list is the only way to preload pools; the test ties it to the directory. All game cues already run inside click / release handlers (user activation), so the prime is belt and braces for browsers that tie permission to the element (iOS Safari); doing it muted keeps "muted means no sound" true even if the flag is on.
- considered: Web Audio API with decoded buffers (more code, and the acceptance is written around `Audio` pools); a JSON manifest of sounds (new content file, and content is read-only in M1); priming only one element per channel (not enough on browsers that gate per element).
- reversible: yes.

## 2026-09-25 — Mute toggle: white 48px disc top-right, icon in text colour, flag stored as 'true'/'false' (B-108)
- chose: `#mute-btn` is a 48x48 white surface disc with the DESIGN shadow at the right end of the header (the title takes `margin-right: auto`, so an auth pill, when logged in, sits between title and toggle). Two inline SVGs: a speaker with two waves (sound on, text colour) and a speaker with a cross (muted, muted colour); the button toggles class `muted`, `aria-pressed` and `aria-label` "Sound on" / "Sound off". `state.muted` (default false) is set from `localStorage['flip.muted']` at start-up and written back by `audio.onState` whenever it changes; the stored value is the string `'true'` or `'false'`, anything else reads as sound on. Turning mute on stops whatever is playing. The CSS cache-buster is bumped to `?v=7` as earlier items did.
- because: DESIGN says sound is on by default and the toggle lives in the header top-right; inline SVG for the same reason as the hearts (Android colour-emoji fallback); the text and muted colours are the palette's natural "on" and "off" pair, and a white disc reads as a tile, matching the paper look. Persisting from the audio subscriber keeps main.js from knowing about storage.
- considered: a primary-teal button (would compete with the theme cards); storing `'1'`/`'0'`; reading storage inside state.js (state.js stays a plain store).
- reversible: yes.
